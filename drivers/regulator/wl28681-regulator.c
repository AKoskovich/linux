...
static struct wl28681 *default_wl2868;
...
static int wl28681_i2c_probe(struct i2c_client *client)
{
	...
	if (atomic_add_return(1, &probe_cnt) > 1) {
		do {
			msleep(20);
			sleep_cnt++;
			dev_info(dev,
	...
	if (default_wl2868 != NULL) {
		wl28681_i2c_disable(default_wl2868);
		dev_info(dev, "Disable default wl2868 I2C start\n");
	}

	wl28681_reset(wl28681);

	if (client->addr != WL28681_DEFAULT_SLAVE_ADDR) {
	...
	if (default_wl2868 != NULL) {
		wl28681_i2c_enable(default_wl2868);
	...
	if (client->addr == WL28681_DEFAULT_SLAVE_ADDR)
		default_wl2868 = wl28681;

	/* Initial related mask for interrupt here */
	...
	for (i = 0; i < ARRAY_SIZE(wl28681->ldo_vout); i++) {
		regmap_read(wl28681->regmap, WL28681_LDO1_VOUT + i,
			    &wl28681->ldo_vout[i]);
		dev_info(dev, "wl28681 ldo%d value is 0x%x\n", i,
			 wl28681->ldo_vout[i]);
	}
	atomic_sub(1, &probe_cnt);
