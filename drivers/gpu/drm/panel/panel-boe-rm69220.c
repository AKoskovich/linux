 ...
 * Copyright (c) 2025 Alexander Koskovich <akoskovich@pm.me>
 */
...
static const struct regulator_bulk_data boe_rm69220_supplies[] = {
	{ .supply = "vddio" },
	{ .supply = "dvdd" },
	{ .supply = "vci" },
};
...
static inline
struct boe_rm69220 *to_boe_rm69220(struct drm_panel *panel)
{
	return container_of(panel, struct boe_rm69220, panel);
}

static void boe_rm69220_reset(struct boe_rm69220 *ctx)
{
...
static int boe_rm69220_on(struct boe_rm69220 *ctx)
{
...
static int boe_rm69220_off(struct boe_rm69220 *ctx)
{
	struct mipi_dsi_multi_context dsi_ctx = { .dsi = ctx->dsi };
...
static int boe_rm69220_prepare(struct drm_panel *panel)
{
	struct boe_rm69220 *ctx = to_boe_rm69220(panel);
	struct device *dev = &ctx->dsi->dev;
	struct drm_dsc_picture_parameter_set pps;
	int ret;

	ret = regulator_bulk_enable(ARRAY_SIZE(boe_rm69220_supplies), ctx->supplies);
	if (ret < 0) {
		dev_err(dev, "Failed to enable regulators: %d\n", ret);
		return ret;
	}

	boe_rm69220_reset(ctx);

	ret = boe_rm69220_on(ctx);
	if (ret < 0) {
		dev_err(dev, "Failed to initialize panel: %d\n", ret);
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		regulator_bulk_disable(ARRAY_SIZE(boe_rm69220_supplies), ctx->supplies);
		return ret;
	}
...
static int boe_rm69220_unprepare(struct drm_panel *panel)
{
	struct boe_rm69220 *ctx = to_boe_rm69220(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	ret = boe_rm69220_off(ctx);
	if (ret < 0)
...
static const struct drm_display_mode boe_rm69220_mode = {
...
static int boe_rm69220_get_modes(struct drm_panel *panel,
					    struct drm_connector *connector)
{
	return drm_connector_helper_get_modes_fixed(connector, &boe_rm69220_mode);
}

static const struct drm_panel_funcs boe_rm69220_panel_funcs = {
	.prepare = boe_rm69220_prepare,
	.unprepare = boe_rm69220_unprepare,
	.get_modes = boe_rm69220_get_modes,
};

static int boe_rm69220_bl_update_status(struct backlight_device *bl)
{
...
static int boe_rm69220_bl_get_brightness(struct backlight_device *bl)
{
	struct mipi_dsi_device *dsi = bl_get_data(bl);
	u16 brightness;
	int ret;
...
static const struct backlight_ops boe_rm69220_bl_ops = {
	.update_status = boe_rm69220_bl_update_status,
	.get_brightness = boe_rm69220_bl_get_brightness,
};

static struct backlight_device *
boe_rm69220_create_backlight(struct mipi_dsi_device *dsi)
{
	...
	return devm_backlight_device_register(dev, dev_name(dev), dev, dsi,
					      &boe_rm69220_bl_ops, &props);
}

static int boe_rm69220_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct boe_rm69220 *ctx;
	int ret;

	ctx = devm_drm_panel_alloc(dev, struct boe_rm69220, panel,
				   &boe_rm69220_panel_funcs,
				   DRM_MODE_CONNECTOR_DSI);
	if (IS_ERR(ctx))
		return PTR_ERR(ctx);

	ret = devm_regulator_bulk_get_const(dev,
					    ARRAY_SIZE(boe_rm69220_supplies),
					    boe_rm69220_supplies,
					    &ctx->supplies);
	if (ret < 0)
		return ret;
	...
	ctx->panel.backlight = boe_rm69220_create_backlight(dsi);
...
static void boe_rm69220_remove(struct mipi_dsi_device *dsi)
{
	struct boe_rm69220 *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;
...
static const struct of_device_id boe_rm69220_of_match[] = {
	{ .compatible = "boe,rm69220" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, boe_rm69220_of_match);

static struct mipi_dsi_driver boe_rm69220_driver = {
	.probe = boe_rm69220_probe,
	.remove = boe_rm69220_remove,
	.driver = {
		.name = "panel-boe-rm69220",
		.of_match_table = boe_rm69220_of_match,
	},
};
module_mipi_dsi_driver(boe_rm69220_driver);

MODULE_AUTHOR("Alexander Koskovich <akoskovich@pm.me>");
MODULE_DESCRIPTION("DRM driver for BOE RM69220 based MIPI DSI panels");
MODULE_LICENSE("GPL");
